...
def is_criticality_balanced(temperature, neutrons_emitted):
    ...
    :param temperatura: temperature value in kelvin
(integer or float)
    :param neutrons_emitted: number of neutrons emitted 
per second (integer or float)
    :return: boolean True if conditions met, False if not
    
    A reactor is said to be critical if it satisfies the 
following conditions:
    - The temperature is less than 800 K.
    - The number of neutrons emitted per second is greater than 500.
    - The product of temperature and neutrons emitted per 
second is less than 500000.
    """
...
def reactor_efficiency(voltage, current, theoretical_max_power):
    """ Asssess reactor efficiency zone.
...
to a 100% efficiency (integer or float)
    :return: str one of 'green', 'orange', 'red', or 
'black'
    ...
    4. black -> less than 40% efficient.
...
def fail_safe(temperature, neutrons_produced_per_econd, threshold):
...
(integer or float)
    ...
    - `temperature * neutrons per second` < 90% of 
`threshold` == 'LOW'
    - `temperature * neutrons per second` +/- 10% of 
`threshold` == 'NORMAL'
    - `temperature * neutrons per second` is not in the 
above-stated ranges == 'DANGER'
    """

    output = temperature * neutrons_produced_per_econd
